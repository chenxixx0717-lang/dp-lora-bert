...
def method_cfg(method: str):
    # shared-ffn 采用 shared_right + lora_modules=ffn 的最接近实现
    # shared_modules 在当前代码中建议使用 attn 或 attn,ffn
    mapping = {
        "all_layer_r5": {
        ...
        "independent_ffn_r5": {
            "route_method": "independent_ffn_only",
            "lora_mode": "standard",
            "shared_modules": "attn",
            "lora_modules": "ffn",
            "active_layers": "all",
        ...
        "shared_ffn_r5": {
            "route_method": "shared_ffn_only",
            "lora_mode": "shared_right",
            "shared_modules": "attn,ffn",
            "lora_modules": "ffn",
            "active_layers": "all",
        },
    }
...
def build_sess(method: str, rank: int, seed: int, sess_suffix: str):
    base = f"dp_task=QNLI_method={method}_rank={rank}_seed={seed}"
    if sess_suffix:
...
def main():
    ...
    parser.add_argument("--sess_suffix", type=str, default="qnli_focus_r1")
    ...
    for method in methods:
        for seed in args.seed_list:
            cmd, sess, cfg = build_cmd(args, method, args.rank, seed)
            jobs.append((method, cfg, args.rank, seed, sess, cmd))

    print(f"Total jobs: {len(jobs)}")
    rows = []
    for i, (method, cfg, rank, seed, sess, cmd) in enumerate(jobs, 1):
                ...
                rows.append(
                    {
                        "task": "QNLI",
                        "method": method,
                        "route_method": cfg["route_method"],
                        "lora_mode": cfg["lora_mode"],
                        "shared_modules": cfg["shared_modules"],
    ...
    out_csv = base_dir / "qnli_focus_train_plan.csv"
    write_plan(rows, out_csv)
    print(f"\nSaved plan/status: {out_csv}")
    print("sess format: dp_task=QNLI_method=<method>_rank=<rank>_seed=<seed>[_<sess_suffix>]")
