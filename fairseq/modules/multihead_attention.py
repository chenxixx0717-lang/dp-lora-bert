...
def multi_head_attention_forward(query,                           # type: Tensor
    ...
    if getattr(args, "debug_lora_forward_once", False):
        cnt = getattr(ma_module, "_debug_lora_attn_in_cnt", 0)
        ma_module._debug_lora_attn_in_cnt = cnt + 1
        if cnt < 2:
            step = cnt + 1
            mods = set(x.strip() for x in str(getattr(args, "lora_modules", "attn,ffn")).split(",") if x.strip())
            print(
                "[DEBUG-LORA-FWD] step=%d module=attn in_proj_lora_norm=%.6e expected_nonzero=%s"
                % (step, lora_acti0.detach().norm().item(), "attn" in mods)
            )
    ...
    if getattr(args, "debug_lora_forward_once", False):
        cnt = getattr(ma_module, "_debug_lora_attn_out_cnt", 0)
        ma_module._debug_lora_attn_out_cnt = cnt + 1
        if cnt < 2:
            step = cnt + 1
            mods = set(x.strip() for x in str(getattr(args, "lora_modules", "attn,ffn")).split(",") if x.strip())
            print(
                "[DEBUG-LORA-FWD] step=%d module=attn out_proj_lora_norm=%.6e expected_nonzero=%s"
                % (step, lora_acti1.detach().norm().item(), "attn" in mods)
            )
