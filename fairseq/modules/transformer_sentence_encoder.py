...
class TransformerSentenceEncoder(nn.Module):
    ...
    def __init__(
        ...
        self._maybe_enable_shared_right()
    ...
    def _maybe_enable_shared_right(self):
        lora_mode = getattr(self.args, "lora_mode", "standard")
        if lora_mode != "shared_right":
            return
        shared_modules = getattr(self.args, "shared_modules", "attn")
        modules = {x.strip() for x in str(shared_modules).split(",") if x.strip()}
        if len(self.layers) <= 1:
            return
        if "attn" in modules:
            base_attn = self.layers[0].self_attn
            shared_in_right = base_attn.in_proj_right.weight
            shared_out_right = base_attn.out_proj_right.weight
            for i in range(1, len(self.layers)):
                self.layers[i].self_attn.in_proj_right.weight = shared_in_right
                self.layers[i].self_attn.out_proj_right.weight = shared_out_right
        if "ffn" in modules:
            base_layer = self.layers[0]
            shared_fc1_right = base_layer.fc1_right.weight
            shared_fc2_right = base_layer.fc2_right.weight
            for i in range(1, len(self.layers)):
                self.layers[i].fc1_right.weight = shared_fc1_right
                self.layers[i].fc2_right.weight = shared_fc2_right
    ...
    def forward(
        ...
        x = self.embed_out(x)   # hook point: embedding output (B, T, C)
