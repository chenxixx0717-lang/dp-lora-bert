...
class TransformerSentenceEncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.args = args
        self.attn_out = nn.Identity()  # hook point: attn branch output (pre-residual)
        self.ffn_out  = nn.Identity()  # hook point: ffn branch output (pre-residual)
    ...
    def forward(
        ...
        x = self.attn_out(x)  # hook point: (T, B, C)
        ...
        if getattr(self.args, "debug_lora_forward_once", False):
            cnt = getattr(self, "_debug_lora_ffn_fc1_cnt", 0)
            self._debug_lora_ffn_fc1_cnt = cnt + 1
            if cnt < 2:
                step = cnt + 1
                mods = set(x.strip() for x in str(getattr(self.args, "lora_modules", "attn,ffn")).split(",") if x.strip())
                print(
                    "[DEBUG-LORA-FWD] step=%d module=ffn fc1_lora_norm=%.6e expected_nonzero=%s"
                    % (step, lora_x.detach().norm().item(), "ffn" in mods)
                )
        ...
        if getattr(self.args, "debug_lora_forward_once", False):
            cnt = getattr(self, "_debug_lora_ffn_fc2_cnt", 0)
            self._debug_lora_ffn_fc2_cnt = cnt + 1
            if cnt < 2:
                step = cnt + 1
                mods = set(x.strip() for x in str(getattr(self.args, "lora_modules", "attn,ffn")).split(",") if x.strip())
                print(
                    "[DEBUG-LORA-FWD] step=%d module=ffn fc2_lora_norm=%.6e expected_nonzero=%s"
                    % (step, lora_x.detach().norm().item(), "ffn" in mods)
                )
        residual_x = self.fc2(x)
        x = lora_x + residual_x
        x = self.ffn_out(x)   # hook point: (T, B, C)
        ################
