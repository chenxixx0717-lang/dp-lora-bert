...
def batch_by_size(
    ...
    """
    def batch_by_size_py(indices_arr, num_tokens, max_tok, max_sent, bsz_mult):
        batches = []
        batch = []
        max_len = 0
        i = 0
        ...
        while i < n:
            ...
            nt = int(num_tokens(idx))
            cand_max_len = max(max_len, nt)
            cand_size = len(batch) + 1
            cand_tokens = cand_max_len * cand_size
            overflow = cand_size > max_sent or cand_tokens > max_tok
            if overflow and len(batch) > 0:
                if bsz_mult > 1 and len(batch) >= bsz_mult:
                    usable = (len(batch) // bsz_mult) * bsz_mult
                    if usable == 0:
                        usable = len(batch)
                    batches.append(batch[:usable])
                    batch = batch[usable:]
                else:
                    batches.append(batch)
                    batch = []
                max_len = 0 if not batch else max(int(num_tokens(j)) for j in batch)
                continue
            batch.append(idx)
            max_len = cand_max_len
            i += 1
        if batch:
            batches.append(batch)
        return batches
    ...
    try:
        ...
        print('| WARNING: fairseq.data.data_utils_fast is not built; using Python fallback batcher (slower).')
        max_tokens = max_tokens if max_tokens is not None else sys.maxsize
        max_sentences = max_sentences if max_sentences is not None else sys.maxsize
        bsz_mult = required_batch_size_multiple
        if isinstance(indices, types.GeneratorType):
            indices = np.fromiter(indices, dtype=np.int64, count=-1)
        return batch_by_size_py(indices, num_tokens_fn, max_tokens, max_sentences, bsz_mult)
